	...
	for (auto tokens : tokensList) {
		if (tokens.front().token == Grammar::LABEL) {
			if (symbols.count(tokens.front().value) == 0) {
				...
				throw std::invalid_argument(std::string("Parsing error at line ") +
				                            std::to_string(tokens.front().line) + ", position " +
				                            std::to_string(tokens.front().position) + ": The label " +
...
Instructions Parser::generateInstructions(TokensList tokensList, SymMap symbols) {
	...
	for (auto tokens : tokensList) {
		...
		auto semicolonpos = find_if(tokens.begin(), tokens.end(),
		                            [](const Token &t) -> bool { return t.token == Grammar::SEMICOLON_SYM; });
